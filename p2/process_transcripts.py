...
def process_transcripts(fname):
    ...
    currSpeaker = None  # "CHATBOT" or "USER"

    for line in f_in:
        stripped = line.strip()

        if stripped.startswith("CHATBOT:"):
            currSpeaker = "CHATBOT"
            continue
        ...
        content = line.rstrip("\n")
        f_out_all.write(content + "\n")

        if currSpeaker == "CHATBOT":
            f_out_chatbot.write(content + "\n")
        elif currSpeaker == "USER":
            f_out_user.write(content + "\n")
